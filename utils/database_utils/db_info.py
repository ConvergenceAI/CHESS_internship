...
def get_db_all_tables(db_path: str) -> List[str]:
    ...
    """
    try:
        raw_table_names = execute_query(db_path, "SELECT name FROM sqlite_master WHERE type='table';")
        return [table[0].replace('\"', '').replace('`', '') for table in raw_table_names if
                table[0] != "sqlite_sequence"]
...
def get_table_all_columns(db_path: str, table_name: str) -> List[str]:
    ...
    """
    try:
        table_info_rows = execute_query(db_path, f"PRAGMA table_info(`{table_name}`);")
