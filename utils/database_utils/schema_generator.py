...
from utils.database_utils.schema import DatabaseSchema, get_primary_keys
...
class DatabaseSchemaGenerator:
    ...
    def __init__(self, tentative_schema: Optional[DatabaseSchema] = None,
                 schema_with_examples: Optional[DatabaseSchema] = None,
                 schema_with_descriptions: Optional[DatabaseSchema] = None, db_id: Optional[str] = None,
    ...
    def _set_primary_keys(db_path: str, database_schema: DatabaseSchema) -> None:
        ...
        schema_with_primary_keys = {
            table_name: {
                col[1]: {"primary_key": True} for col in execute_query(db_path, f"PRAGMA table_info(`{table_name}`)") if
                col[5] > 0
            }
    ...
    def _set_foreign_keys(db_path: str, database_schema: DatabaseSchema) -> None:
        ...
        for table_name, columns in schema_with_references.items():
            foreign_keys_info = execute_query(db_path, f"PRAGMA foreign_key_list(`{table_name}`)")
            for fk in foreign_keys_info:
                ...
                destination_column = get_primary_keys(database_schema.tables[destination_table])[0] if not fk[
                    4] else database_schema.get_actual_column_name(fk[2], fk[4])

                schema_with_references[source_table][source_column]["foreign_keys"].append(
                    (destination_table, destination_column))
                schema_with_references[destination_table][destination_column]["referenced_by"].append(
    ...
    def _load_schema_into_cache(cls, db_id: str, db_path: str) -> None:
        ...
        schema_with_type = {
            table_name: {col[1]: {"type": col[2]} for col in
                         execute_query(db_path, f"PRAGMA table_info(`{table_name}`)", fetch="all")}
    ...
    def _load_table_and_column_info(self) -> None:
        ...
        """
        self.schema_structure = DatabaseSchemaGenerator.CACHED_DB_SCHEMA[self.db_id].subselect_schema(
        ...
        for table_name, table_schema in self.schema_structure.tables.items():
            for column_name, column_info in table_schema.columns.items():
                if (self.add_examples and not column_info.examples) or (
                    ...
                    example = execute_query(db_path=self.db_path,
    ...
    def _load_column_descriptions(self) -> None:
        ...
        """
        self.schema_structure.add_info_from_schema(self.schema_with_descriptions,
                                                   field_names=["original_column_name", "column_name",
    ...
    def _extract_create_ddl_commands(self) -> Dict[str, str]:
        ...
        for table_name in self.schema_structure.tables.keys():
            create_prompt = execute_query(db_path=self.db_path,
    ...
    def _get_example_column_name_description(self, table_name: str, column_name: str,
                                             include_value_description: bool = False) -> str:
        """
        ...
        for table_name, table_schema in self.schema_structure.tables.items():
            ...
            for column_name, column_info in table_schema.columns.items():
                if with_keys or not (column_info.primary_key or column_info.foreign_keys or column_info.referenced_by):
                    ...
                    if (column_info.column_name.lower().strip() != column_name.lower().strip()) and (
