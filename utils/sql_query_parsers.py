...
from utils.database_utils.db_info import get_table_all_columns, get_db_all_tables
...
def get_sql_columns_dict(db_path: str, sql: str) -> Dict[str, List[str]]:
    ...
    """
    sql = qualify(parse_one(sql, read='sqlite'), qualify_columns=True, validate_qualify_columns=False) if isinstance(
        sql, str) else sql
    columns_dict = {}
            ...
            for literal in where_exp.find_all(exp.Literal):
                if literal == literal.parent.expression:
                    for column_exp in literal.parent.find_all(exp.Column):
                        ...
                        table_name = next(
                            (table for table, columns in columns_dict.items() if
                             column_name.lower() in [c.lower() for c in columns]), None)
                        if table_name:
...
def _check_value_exists(db_path: str, table_name: str, column_name: str, value: str) -> Optional[str]:
    ...
    result = execute_query(db_path, query, "one")
    return result[0] if result else None


def get_sql_condition_literals_existent(db_path: str, sql: str) -> Dict[str, Dict[str, List[str]]]:
    """
                        ...
                        for table_name, column_names in columns_dict.items():
                            if column_name.lower() in [col.lower() for col in column_names]:
                                ...
                                if "LIKE" in str(literal.parent):
                                    ...
                                    value_check = _check_value_exists(db_path, table_name, column_name,
                                                                      example_to_search)
                                    if value_check:
                                        example_exist = True
